...
//#include "coax_msgs/CoaxExternalState.h"
...
#define COPY(V) stateMsg.V = state->V
			...
			for (unsigned int i=0;i<3;i++) {
				COPY(gyro[i]);
				COPY(accel[i]);
                unsigned short mag = ((unsigned short)(state->magneto[i]));
                stateMsg.magneto[i] = (signed short)mag;
				COPY(o_attitude[i]);
			}
			COPY(imutemp);
			COPY(zrange);
...
//		void external_state_callback(const coax_msgs::CoaxExternalState::ConstPtr & message) {
//			DEBUG(res = sbSimpleSetExternalState(simple,message->input_selector,message->yaw,message->altitude));
//		}
